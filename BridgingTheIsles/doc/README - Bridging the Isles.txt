...
2. Once the game starts, you (a stick figure) are automatically placed on the first isle with the second one to the right. The points on the top left corner display 0, and the lives display 2.
...
12. If you still have lives remaining, you are placed back in the starting position and can continue the game. If you have no more lives, you are reverted back to the starting screen. Your point score is displayed at the top.
**Click on the ‘Quit” button on the top right hand corner to exit the game. You go back to the
starting screen to try the other level of just stop completely
**Click on the “restart” button to go back to the starting point of the level
...
* (DONE) The player can build a bridge by pressing the space button, and the longer the player presses, the longer the bridge gets.[f][g][h][i]
...
* OriginalBridge (superclass) - a graphical component that is vertically “built” by the player when he/she presses the space button and horizontally falls to connect the Person to the adjacent 
Isle
...
* Riya:
   ...
   * Setting up game to run with different screens
   * Classes: DrawingSurface, GameScreen, Main, MenuScreen, Screen, ScreenSwitcher
* Kasturi: 
...
* Samantha:
   ...
   * Classes: Person, OriginalBridge, Isle, GameScreen, PointSystem, LifeCounter, MenuScreen


* Using the Processing library for GUI methods
* Used Mr. Shelby’s ProcessingScreenSwitching project to guide the menu graphics
